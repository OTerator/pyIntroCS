...
# uniqueElements([1, 2, 3, 4, 5], [4, 5, 6, 7, 8])



#9
def unionOfSets(sLst):
    return {n for s in sLst for n in s}
        
# print(unionOfSets([{1, 2}, {2, 3, 4}, {4, 5}]))



#10
def commonElements(sLst):
...
# print(commonElements([{1, 2, 3}, {2, 3, 4}, {2, 3, 4, 5}]))
    


#11
def uniqueInEachSet(sLst):
    
    n = set() #intersection including all elements which are not exclusive.
    
    for i in range(len(sLst)):
        n |= sLst[i-1] & sLst[i]
        
    return [s-n for s in sLst]

# print(uniqueInEachSet([{1, 2, 3}, {2, 4, 5}, {3, 5, 6, 7}]))



#12
dct = ({1:{2,3,5}, 2:{1,4}, 3:{1,5}, 4:{2,5,1}, 5:{3,4,2}})

def isSetDict(dct):
    
    return {type(v)==set for v in dct.values()} == {True}
...
def twinsDict(d):
    if isSetDict(d) == False:
        return TypeError, "Dictionary values must consist of sets only."
